...
def alphaBetaSearch(game, state):
    ...
    for action in game.actions(state):
        ...
        # MAX 走完一步，下一层轮到 MIN
        value = minValue(game, next_state, root_player, alpha, beta)
...
def maxValue(game: AdversarialGame, state, root_player, alpha, beta):
    """
    当前 state 是 MAX 节点。
    返回这个状态的 Minimax 值。
    """
    ...
    for action in game.actions(state):
        next_state = game.result(state, action)
        value = max(value, minValue(game, next_state, root_player, alpha, beta))
...
def minValue(game, state, root_player, alpha, beta):
    """
    当前 state 是 MIN 节点。
    返回这个状态的 Minimax 值。
    """
    ...
    for action in game.actions(state):
        next_state = game.result(state, action)
        value = min(value, maxValue(game, next_state, root_player, alpha, beta))
